...
bool better_candidate(const Candidate &a, const Candidate &b)
{
    ...
    if (a.finish + EPS < b.finish)
        return true;
    if (b.finish + EPS < a.finish)
        return false;
...
string solve_sequential_single_run(mt19937 &rng)
{
    ...
    while (unvisited > 0)
    {
        ...
        while (true)
        {
            ...
            sort(valid_cands.begin(), valid_cands.end(), better_candidate);

            // WIDENED SEARCH: Pick randomly from the Top 5 best candidates
            int top_k = min(5, (int)valid_cands.size());
            uniform_int_distribution<> dist(0, top_k - 1);
            Candidate best = valid_cands[dist(rng)];
...
string solve_simultaneous_single_run(mt19937 &rng)
{
    vector<bool> visited(customers.size(), false);
    visited[0] = true;
    int unvisited = (int)customers.size() - 1;
...
{
    mt19937 rng(2026);
    int best_fleet_size = 1e9;

    // BALANCED: 20 Iterations allows for high speed without losing accuracy
    for (int iter = 0; iter < 20; iter++)
    {
        string res = solve_sequential_single_run(rng);
        if (res != "INFEASIBLE")
        {
            int fleet_used = stoi(res);
            best_fleet_size = min(best_fleet_size, fleet_used);
        }
    }
    return (best_fleet_size == 1e9) ? "INFEASIBLE" : to_string(best_fleet_size);
}
...
string solve_simultaneous()
{
    ...
    for (int iter = 0; iter < 20; iter++)
    {
        string res = solve_simultaneous_single_run(rng);
        if (res != "INFEASIBLE")
        {
            int fleet_used = stoi(res);
            best_fleet_size = min(best_fleet_size, fleet_used);
...
int main()
{
    ...
    cout << "=======================================================\n";
    cout << "  V4 BALANCED (TOP-5 MOVES, 20 ITERATIONS)\n";
    cout << "=======================================================\n";
    p("OVERALL", o);
    p("CLUSTERED (C)", c);
    p("RANDOM (R)", r);
