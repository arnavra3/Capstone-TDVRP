...
Result solve_seqsl(int version, mt19937 &rng)
{
    ...
    sort(local_garage.begin(), local_garage.end()); // SMALL TO LARGE (The defining feature of SEQSL)
...
int main()
{
    ...
    ifstream file("TD_MICRO_11_Instances.txt");
    if (!file)
    {
    ...
    while (load_next_instance(file, layout))
    {
        ...
        // Version 1
        Result r1 = solve_seqsl(1, rng);
        print_version_result(1, r1);

        // Version 2
        Result r2 = solve_seqsl(2, rng);
        print_version_result(2, r2);

        // Version 3
        Result r3 = solve_seqsl(3, rng);
