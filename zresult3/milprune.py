...
def load_global_heuristic_bounds():
    """
    Scans the local directory for the bounds files generated by the C++ scripts.
    Returns a dictionary mapping instance names to the absolute best (minimum) time found.
    """
    bounds = {}
    
    # Get the exact folder where milprune.py lives (e.g., .../zresult3)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Force Python to look inside that specific folder
    files_to_check = [
        os.path.join(script_dir, "seqls_bounds.csv"),
        os.path.join(script_dir, "seqsl_bounds.csv"),
        os.path.join(script_dir, "sim_bounds.csv")
    ]
    
    print("\n[*] Loading heuristic upper bounds from C++ outputs...")
    files_found = 0
    for file in files_to_check:
        if os.path.exists(file):
            ...
            with open(file, 'r') as csvfile:
                ...
                for row in reader:
                    if len(row) == 2:
                        inst, time_val = row[0].strip(), float(row[1])
                        # Keep only the absolute lowest time found across all heuristics
                        if inst not in bounds or time_val < bounds[inst]:
                            bounds[inst] = time_val
    
    if files_found == 0:
        print("[!] Warning: No bounds CSV files found in this directory. Run the C++ heuristics first!")
    else:
        print(f"[+] Loaded tight bounds for {len(bounds)} instances from {files_found} files.")
    return bounds

def calculate_distance(x1, y1, x2, y2):
    return math.hypot(x1 - x2, y1 - y2)

def solve_instance(inst, heuristic_ub=None):
    print(f"\n{'='*50}")
    ...
    objective_expr = sum(t[d] for d in return_depots)
    solver.Minimize(objective_expr)

    # --- DYNAMIC UPPER BOUND PRUNING ---
    if heuristic_ub is not None:
        # Buffer of 0.01 prevents SCIP from pruning true optimal due to float discrepancies
        safe_ub = heuristic_ub + 0.01
        print(f"[*] Pruning active: Branching restricted to solutions better than {safe_ub:.2f}")
        solver.Add(objective_expr <= safe_ub)
...
def parse_and_solve(filename):
    if not os.path.exists(filename):
        print(f"[!] Critical Error: Instances file not found at {filename}")
        return

    with open(filename, 'r') as f:
        lines = [line.strip() for line in f if line.strip()]
    
    # Auto-load the best heuristic bounds generated by your C++ files
    global_heuristic_bounds = load_global_heuristic_bounds()
    ...
    while idx < len(lines):
        ...
        # Get the strict upper bound for this specific instance (if it exists)
        best_ub = global_heuristic_bounds.get(inst['name'], None)
        res = solve_instance(inst, heuristic_ub=best_ub)
        if res:
            results.append(res)
    ...
    # Dynamically build output path
    script_dir = os.path.dirname(os.path.abspath(__file__))
    csv_filename = os.path.join(script_dir, "milp_results_baseline_pruned.csv")
    ...
    print(f"\n[+] Successfully saved heavily pruned benchmark data to {csv_filename}")

if __name__ == '__main__':
    script_dir = os.path.dirname(os.path.abspath(__file__))
    txt_path = os.path.join(script_dir, "TD_MALANDRAKI_32_Instances.txt")
    parse_and_solve(txt_path)