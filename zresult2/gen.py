...
# The exact grid used to generate the 32 test cases in Table V
SIZES = [10, 15, 20, 25]
INTERVAL_TYPES = [2, 3]
REPLICATIONS = 4 # 4 layouts per size/interval combo
NUM_DATASETS = len(SIZES) * len(INTERVAL_TYPES) * REPLICATIONS # Equals 32

def generate_malandraki_32():
    filename = "TD_MALANDRAKI_32_Instances.txt"
    rng = random.Random(1992) # Classic seed
    ...
    with open(filename, "w") as f:
        ...
        # Methodically loop through to create the exact 32-case matrix
        for num_cust in SIZES:
            for num_intervals in INTERVAL_TYPES:
                for rep in range(1, REPLICATIONS + 1):
                    ...
                    name = f"TD_MALANDRAKI_N{num_cust}_INT{num_intervals}_REP{rep}"
    ...
    print(f"Generated exactly {NUM_DATASETS} Malandraki-style instances in {filename}")

if __name__ == "__main__":
    generate_malandraki_32()