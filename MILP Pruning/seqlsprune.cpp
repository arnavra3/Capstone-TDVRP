...
void print_row(string col1, string col2, double dem_cap, const GroupStats& s) {
    cout << left << setw(8) << col1 << setw(6) << col2 << setw(10) << fixed << setprecision(2) << dem_cap;
    if (s.count_v1 > 0) cout << setw(10) << fixed << setprecision(3) << (s.sum_ratio_v1 / s.count_v1); else cout << setw(10) << "N/A";
    cout << setw(8) << fixed << setprecision(2) << (s.inst_count ? s.sum_veh_v1 / s.inst_count : 0.0);
    if (s.count_v2 > 0) cout << setw(10) << fixed << setprecision(3) << (s.sum_ratio_v2 / s.count_v2); else cout << setw(10) << "N/A";
    cout << setw(8) << fixed << setprecision(2) << (s.inst_count ? s.sum_veh_v2 / s.inst_count : 0.0);
    if (s.count_v3 > 0) cout << setw(10) << fixed << setprecision(3) << (s.sum_ratio_v3 / s.count_v3); else cout << setw(10) << "N/A";
    cout << setw(8) << fixed << setprecision(2) << (s.inst_count ? s.sum_veh_v3 / s.inst_count : 0.0);
    if (s.count_v4 > 0) cout << setw(10) << fixed << setprecision(3) << (s.sum_ratio_v4 / s.count_v4); else cout << setw(10) << "N/A";
    cout << setw(8) << fixed << setprecision(2) << (s.inst_count ? s.sum_veh_v4 / s.inst_count : 0.0) << "\n";
}

int main() {
    map<string, double> milp_baselines = load_milp_baselines("milp_results_baseline_pruned.csv");
    ifstream file("TD_MALANDRAKI_32_Instances.txt");
    if (!file) { cout << "Could not open instances file.\n"; return 1; }

    ofstream bounds_file("seqls_bounds.csv"); // --- OUTPUT FILE FOR MILP ---
    mt19937 rng(2026);
    string layout;
    ...
    while (load_next_instance(file, layout)) {
        ...
        // Run all versions and capture results
        Result r1 = solve_seqls(1, rng); update_stats(table_data[intervals][n], r1, best_sol, 1);
        Result r2 = solve_seqls(2, rng); update_stats(table_data[intervals][n], r2, best_sol, 2);
        Result r3 = solve_seqls(3, rng); update_stats(table_data[intervals][n], r3, best_sol, 3);
        ...
        // --- EXPORT BEST BOUND ---
        double instance_best = 1e18;
        if(r1.feasible) instance_best = min(instance_best, r1.total_time);
        if(r2.feasible) instance_best = min(instance_best, r2.total_time);
        if(r3.feasible) instance_best = min(instance_best, r3.total_time);
        if(best_r4.feasible) instance_best = min(instance_best, best_r4.total_time);
        
        if (instance_best < 1e18) {
            bounds_file << instance_name << "," << fixed << setprecision(4) << instance_best << "\n";
        }
    }
    ...
    cout << left << setw(8) << "per Lnk" << setw(6) << "" << setw(10) << "Ratio" 
         << setw(10) << "Heur/Opt" << setw(8) << "Veh" << setw(10) << "Heur/Opt" << setw(8) << "Veh"
         << setw(10) << "Heur/Opt" << setw(8) << "Veh" << setw(10) << "Heur/Opt" << setw(8) << "Veh\n";
    cout << "-------------------------------------------------------------------------------------------------------\n";
