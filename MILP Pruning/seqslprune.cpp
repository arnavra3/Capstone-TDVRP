...
Result solve_seqsl(int version, mt19937 &rng) {
    ...
    sort(local_garage.begin(), local_garage.end()); // SMALL TO LARGE (SEQ-SL)
...
int main() {
    ...
    ofstream bounds_file("seqsl_bounds.csv"); // --- OUTPUT FILE FOR MILP ---
    mt19937 rng(2026);
    string layout;
    ...
    while (load_next_instance(file, layout)) {
        ...
        Result r1 = solve_seqsl(1, rng); update_stats(table_data[intervals][n], r1, best_sol, 1);
        Result r2 = solve_seqsl(2, rng); update_stats(table_data[intervals][n], r2, best_sol, 2);
        Result r3 = solve_seqsl(3, rng); update_stats(table_data[intervals][n], r3, best_sol, 3);
        
        Result best_r4;
        for (int i = 0; i < 20; i++) {
            Result r = solve_seqsl(4, rng);
