...
void update_stats(GroupStats &stats, const Result &r, double best_sol, bool is_probabilistic) {
    if (r.feasible) {
        double veh = r.fleet.size();
        if (best_sol > 0) {
            double ratio = r.total_time / best_sol;
            if (!is_probabilistic) { stats.count_sim++; stats.sum_ratio_sim += ratio; }
            else { stats.count_prob++; stats.sum_ratio_prob += ratio; }
        }
        if (!is_probabilistic) stats.sum_veh_sim += veh;
        else stats.sum_veh_prob += veh;
    }
}
...
void print_row(string col1, string col2, double dem_cap, const GroupStats& s) {
    ...
    if (s.count_sim > 0) cout << setw(10) << fixed << setprecision(3) << (s.sum_ratio_sim / s.count_sim); else cout << setw(10) << "N/A";
    cout << setw(12) << fixed << setprecision(2) << (s.inst_count ? s.sum_veh_sim / s.inst_count : 0.0);
    if (s.count_prob > 0) cout << setw(10) << fixed << setprecision(3) << (s.sum_ratio_prob / s.count_prob); else cout << setw(10) << "N/A";
    cout << setw(8) << fixed << setprecision(2) << (s.inst_count ? s.sum_veh_prob / s.inst_count : 0.0) << "\n";
}
...
int main() {
    ...
    ofstream bounds_file("sim_bounds.csv"); // --- OUTPUT FILE FOR MILP ---
    mt19937 rng(2026);
    string layout;
    ...
    while (load_next_instance(file, layout)) {
        ...
        Result r_sim = solve_sim(false, rng);
        update_stats(table_data[intervals][n], r_sim, best_sol, false);
        ...
        if(r_sim.feasible) instance_best = min(instance_best, r_sim.total_time);
        if(best_prob.feasible) instance_best = min(instance_best, best_prob.total_time);
    ...
    cout << left << setw(8) << "per Lnk" << setw(6) << "" << setw(10) << "Ratio" 
         << setw(10) << "Heur/Opt" << setw(12) << "Vehicles" << setw(10) << "Heur/Opt" << setw(8) << "Vehicles\n";
    cout << "---------------------------------------------------------------------------------\n";
