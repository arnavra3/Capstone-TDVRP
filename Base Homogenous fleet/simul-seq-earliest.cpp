...
//EARLIEST START TIME OF NEXT NODE
...
// =======================================================
// V2 SMART LOGIC: Absolute Earliest Start Time
// =======================================================
bool better_candidate(const Candidate &a, const Candidate &b)
{
    ...
    // 1. PRIMARY: Absolute Earliest Start Time.
    // This perfectly evaluates Travel + Wait, AND allows Simultaneous to use parallel trucks!
    if (a.start + EPS < b.start)
        return true;
    if (b.start + EPS < a.start)
        return false;

    // 2. SECONDARY: Tie-breaker! If start times are the same, prefer the ACTIVE truck.
    // This stops it from spawning a new truck when the current truck can just wait in the lot.
    if (a.vehicle_idx != -2 && b.vehicle_idx == -2)
        return true;
    if (a.vehicle_idx == -2 && b.vehicle_idx != -2)
        return false;

    // 3. TERTIARY: If still perfectly tied, use the shortest physical travel time
...
int main()
{
    ...
    cout << "=======================================================\n";
    cout << "  V2 SMART BASELINE (EARLIEST START TIME)\n";
    cout << "=======================================================\n";
    p("OVERALL", o);
    p("CLUSTERED (C)", c);
    p("RANDOM (R)", r);
